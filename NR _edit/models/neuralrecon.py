...
class NeuralRecon(nn.Module):
    ...
    def forward(self, inputs, save_mesh=False):
        ...
        # Compute the losses
        albedo_loss = torch.sum((albedo - albedoBatch) * (albedo - albedoBatch))
        normal_loss = torch.sum((normal - normalBatch) * (normal - normalBatch))
        rough_loss = torch.sum((rough - roughBatch) * (rough - roughBatch))
        depth_loss = torch.sum((torch.log(depth + 1) - torch.log(depthBatch + 1))
                    * (torch.log(depth + 1) - torch.log(depthBatch + 1)))

        # Compute error using BatchNorm. Not trying it right now.
