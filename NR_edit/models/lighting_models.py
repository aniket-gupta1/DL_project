...
class Decoder(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.mode == 0:
            out = torch.clamp(1.01 * torch.tanh(out), -1, 1)
        elif self.mode == 1:
            x_orig = torch.clamp(1.01 * torch.tanh(out), -1, 1)
            norm = torch.sqrt(torch.sum(x_orig * x_orig, dim=1).unsqueeze(1) ).expand_as(x_orig)
            out = x_orig / torch.clamp(norm, min=1e-6)
        elif self.mode == 2:
            x_orig = torch.clamp(1.01 * torch.tanh(out), -1, 1)
            out = torch.mean(x_orig, dim=1).unsqueeze(1)
        elif self.mode == 3:
            out = F.softmax(out, dim=1)
        elif self.mode == 4:
            x_orig = torch.mean(out, dim=1).unsqueeze(1)
            out = torch.clamp(1.01 * torch.tanh(x_orig), -1, 1)
